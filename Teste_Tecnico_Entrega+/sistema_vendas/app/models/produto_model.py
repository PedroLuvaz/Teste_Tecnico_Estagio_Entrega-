...
class ProdutoModel:
    ...
    def get_by_id(self, produto_id):
        ...
        with conn.cursor() as cur:
            cur.execute("SELECT id, nome, preco, categoria, estoque FROM produtos WHERE id = %s", (produto_id,))
            return cur.fetchone()
    ...
    def get_by_category(self, categoria):
        ...
        if not conn: return []
        with conn.cursor() as cur:
            cur.execute("SELECT id, nome, preco, categoria, estoque FROM produtos WHERE categoria ILIKE %s", (f'%{categoria}%',))
            return cur.fetchall()
